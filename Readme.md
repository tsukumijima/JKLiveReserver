...
> 予約した番組の概要欄は公式実況チャンネルのフォーマットにならい、公式実況チャンネルも含めた全ての実況チャンネルの一覧をチャンネル/コミュニティページへのリンクつきで設定されます。
